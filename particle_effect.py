...
class GravityParticleEffect():
    def __init__(self, max_radius:float, loc:tuple, width:float, color:tuple=Color.GRAVITY_PARTICLE_COLOR):
    ...
    def render(self, screen, zero_vector:tuple, unit_size:float):
        pygame.draw.circle(screen, self.__color, change_normalized_into_real(zero_vector, unit_size, self.__loc), 5, 0)
        pygame.draw.circle(screen, self.__color, change_normalized_into_real(zero_vector, unit_size, self.__loc), self.__radius, self.__width)
        self.__radius -= 0.5
        if self.__radius <= 0:
